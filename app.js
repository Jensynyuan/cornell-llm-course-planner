(() => {
  ...
  const CORNELL_LLM_REGISTRATION_POLICY = Object.freeze({
    minimum:10,
    maximum:17,
    sourceLabel:"Cornell Law 2026–27 Student Handbook · General LL.M. Program",
    sourceUrl:"https://publications.lawschool.cornell.edu/student-handbook/academic-degree-requirements/llm/"
  });
  ...
  const RELEASE_NOTES = [
    { version:"v5.34", zh:["按 Cornell Law 2026–27 General LL.M. Student Handbook 将每学期注册范围由旧版 10–15 更正为 10–17 学分；16 学分不再被误报为超学分。","IALS（LAW 6091）仍按 Fall 注册课程计入学期总注册学分，但不计入 General LL.M. 学位要求的 20 个其他法学分。","新增政策来源与冲突提示：当前学年专门 Handbook 明确为 10–17；Registrar 通用 FAQ 仍显示 10–15，如 Student Center 实际拦截应向 Registrar／Dean of Students 确认。"], en:["Corrected the per-semester General LL.M. registration range from the outdated 10–15 rule to 10–17 credits under Cornell Law's 2026–27 Student Handbook; a 16-credit semester is no longer misreported as an overload.","IALS (LAW 6091) remains included in Fall registered credits, while remaining excluded from the separate 20-credit General LL.M. degree-course requirement.","Added policy provenance and a source-conflict notice: the year-specific Handbook states 10–17, while the generic Registrar FAQ still says 10–15; confirm with the Registrar or Dean of Students if Student Center applies a lower operational cap."] },
  ...
  const FAQ_ITEMS = [
    ...
    {
      category: "选课",
      question: "2026–27 General LL.M. 每学期上限是 15 还是 17 学分？",
      answer: "2026–27 General LL.M. Student Handbook 明确规定每学期至少注册 10 学分、最多注册 17 学分，超出该范围须取得 Dean of Students 批准。IALS 是 Fall 的 2 学分注册课程，因此会计入 Fall 总注册学分；它只是在计算学位要求的 20 个其他法学分时被排除。旧版 Fall 2025 指引及当前 Registrar 通用 FAQ 仍写 10–15，软件对 2026–27 学年优先采用该学年专门 Handbook 的 10–17 规则；如 Student Center 实际限制不同，应直接向 Registrar／Dean of Students 确认。",
      sourceLabel: "Cornell Law：2026–27 General LL.M. Student Handbook",
      sourceUrl: CORNELL_LLM_REGISTRATION_POLICY.sourceUrl
    },
    {
      category: "选课",
...
,
  ...
  const FAQ_EN = [
    ...
    { category:"Enrollment", question:"Is the 2026–27 General LL.M. semester limit 15 or 17 credits?", answer:"The 2026–27 General LL.M. Student Handbook states a minimum of 10 and a maximum of 17 registered credits per semester, with Dean of Students approval required outside that range. IALS is a registered 2-credit Fall course and therefore appears in the Fall registration total; it is excluded only from the separate 20-credit degree-course requirement. The older Fall 2025 instructions and the generic Registrar FAQ still say 10–15, so this planner follows the year-specific 2026–27 Handbook and flags the source conflict. Confirm directly if Student Center applies a lower operational cap." },
  ...
  function renderProgress() {
    ...
    if (currentSchoolId === "cornell") {
      ...
      const registrationMinimum = CORNELL_LLM_REGISTRATION_POLICY.minimum;
      const registrationMaximum = CORNELL_LLM_REGISTRATION_POLICY.maximum;
      ...
      els.creditProgress.innerHTML = `<p class="progress-term-context"><strong>${isEnglish() ? "2026–27 academic-year record" : "2026–27 秋春学年个人记录"}</strong><span>${isEnglish() ? "The catalog term filter never removes another term's selections or progress." : "课程库学期筛选不会移除另一学期的选课或进度。"}</span></p>${rangeProgressHtml(isEnglish() ? `Fall 2026 LL.M. registration (${registrationMinimum}–${registrationMaximum})` : `2026 秋季 LL.M. 注册学分（${registrationMinimum}–${registrationMaximum}）`, annual.registrationCredits.FA26, registrationMinimum, registrationMaximum)}${rangeProgressHtml(isEnglish() ? `Spring 2027 LL.M. registration (${registrationMinimum}–${registrationMaximum})` : `2027 春季 LL.M. 注册学分（${registrationMinimum}–${registrationMaximum}）`, annual.registrationCredits.SP27, registrationMinimum, registrationMaximum)}${progressHtml(isEnglish() ? "2026–27 NY Bar qualifying classroom credits" : "2026–27 学年 NY Bar 课堂课程学分", annual.eligibleCredits, 24)}${nyBarCreditBreakdownHtml(selected)}<p class="progress-policy-note">${isEnglish() ? "NY Bar credits and categories aggregate all selected Fall and Spring offerings. For Cornell Law lectures, seminars, and discussions, a blank or not-yet-published instruction mode does not remove classroom credit; an explicit Online or Remote designation does. Independent study does not count, while clinics, practica, hybrid delivery, and restricted sections require confirmation." : "NY Bar 课堂学分及分类统一累计全部秋季与春季已选课程。Cornell Law 的讲授课、研讨课和讨论课不会仅因授课方式留空或尚未发布而被扣除；明确标为 Online／Remote 才会因授课方式不计入。独立研究不计入，诊所、实践课、混合授课及项目限制班次须确认。"}</p><div class="progress-subheading">${isEnglish() ? "NY Bar required categories · full academic year" : "NY Bar 分类要求 · 秋春学年累计"}</div>${barItems.map(([label, value, target]) => progressHtml(label, value, target)).join("")}${barAllocationControlsHtml(selected)}<p class="progress-policy-note">${isEnglish() ? "Where Cornell lists a course in more than one category, the planner counts it once in the category selected above. Confirm your final allocation with Cornell Law and BOLE." : "若 Cornell 将同一课程列入多个类别，工具仅按上方所选归类计入一次；最终学分分配请向 Cornell Law 和 BOLE 确认。"}</p><p class="progress-policy-note">${isEnglish() ? `For 2026–27, the General LL.M. Student Handbook sets a ${registrationMinimum}–${registrationMaximum} credit registration range per semester; approval is required outside that range. IALS counts in the Fall registration total but not toward the separate 20-credit degree-course requirement. Cornell's generic Registrar FAQ still says 10–15, so confirm directly if Student Center applies a lower operational cap. ` : `2026–27 General LL.M. Student Handbook 规定每学期注册 ${registrationMinimum}–${registrationMaximum} 学分，超出范围须申请批准。IALS 计入 Fall 总注册学分，但不计入另行要求的 20 个其他法学分。Cornell Registrar 通用 FAQ 仍写 10–15；如 Student Center 实际采用更低上限，应直接确认。`}<a href="${CORNELL_LLM_REGISTRATION_POLICY.sourceUrl}" target="_blank" rel="noopener">${isEnglish() ? "Official 2026–27 Handbook" : "2026–27 官方 Handbook"}</a></p>`;
  ...
  function rangeProgressHtml(label, value, minimum, maximum) {
    ...
    const status = belowMinimum ? (isEnglish() ? `${creditCount(minimum - value)} below the published minimum` : `距公布下限还差 ${minimum - value} 学分`) : (isEnglish() ? `Within the published ${minimum}–${maximum} credit range` : `处于公布的 ${minimum}–${maximum} 学分范围内`);
